...
{ 	
  ...
  //double delta0;    
  //delta0 = delta;  //uncomment if you want to test optimal delta for MC
  //double temp0=temp;
  //temp0 = temp; //uncomment for equilibr tests of temp => energy
  int nconf = 1;
  //fool cycles to equilibrate
  for(int i=0; i<n_eq ; i++){
	  Move();
  }
  /*for(int i=0; i<20; i++){ //loop over simulations
  Input();
  tot_acc = 0;
  tot_att = 0;
  //delta = delta0 + i*1; //delta test
  temp = temp0 + i*0.003;
  beta = 1/temp;*/
...
//        ConfXYZ(nconf);//Write actual configuration in XYZ format //Commented to avoid "filesystem full"! 
 ...
 //}// mega-loop over simulations for computing equi tests
  return 0;
}
...
//Read input informations
  ...
  if(iNVET) cout << "The program perform Metropolis moves with uniform translations" << endl;
  else cout << "The program perform Molecular Dynamics simulation" << endl;
  cout << "Moves parameter  		=" << delta << endl;
  cout << "Number of blocks 		=" << nblk << endl;
  cout << "Number of steps in one block =" << nstep << endl << endl;
  cout << "Number of equilibration steps=" << n_eq <<endl;
  cout << "Bins for g(r) 		=" << nbins <<endl;
...
//Print initial values for measured properties
  ...
  //getc(stdin); //(un)comment for extra flair
...
void Measure() //Properties measurement
{
  ...
  double vij;
  double dx, dy, dz, dr;
  double tc_vir,tc_v; //tail corrections
...
// distance i-j in pbc
      ...
      if(dr < rcut)
      {
        ...
        v += vij;
        vir += 48*((1.0/pow(dr,12)) - (0.5/pow(dr,6)));
      }
...
//we bin the space
  ...
  tc_v = (npart*8.*M_PI*rho)*(1./(9.*pow(rcut,9.)) -1./(3.*pow(rcut,3.))) ;
  tc_vir = (96*npart*M_PI*rho)*(1./(9.*pow(rcut,9.)) -1./(3.*pow(rcut,3.))) ;
  walker[iv] = (4.0 * v) + tc_v; // Potential energy
  walker[ik] = kin; // Kinetic energy
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
  walker[ie] = 4.0 * v + tc_v + kin;  // Total energy
  walker[ip] = rho*walker[it] + ((vir + tc_vir)/(3*vol)); // rho*walker[it] ; //Pressure 
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres,Acc,Df,Dfin,Equitest;
   //const int wd=2;
    ...
    Acc.open("output_acc.dat",ios::app);
    Df.open("output_g.dat",ios::app);
    Dfin.open("output_gfin.dat",ios::app);
    Equitest.open("equitest.dat",ios::app);
    ...
    rtmp=i*drbin;
    rtmp= (4.*M_PI/3.)*(pow((rtmp+drbin),3.) - pow(rtmp,3.)); //it now holds deltaV
    normhist = rho*npart*rtmp;
    stima_g = g_blk[i]/(blk_norm*normhist); 
...
//g(r)
    ...
    cout << "----------------------------" << endl << endl;
if (iblk == nblk){
//output only on simulation end, e.g. g(r), equitests...
 Acc << delta << ' ' << (double)tot_acc/(double)(tot_att) <<endl;
 for(int i=0; i<nbins; i++){
 	Dfin << i*drbin <<' '<< g_glob[0][i]/(double)iblk <<' '<<Error(g_glob[0][i],g_glob[1][i],nblk) << endl;
 	}
 	Equitest <<temp << ' '<< glob_av[ie]/(double)iblk<<endl;	
}
    Epot.close();
    Ekin.close();
    Etot.close();
